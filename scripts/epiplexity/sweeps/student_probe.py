...
Each config is a separate subprocess invocation of trainers/evolve_gzip.py
...
Outputs land in scripts/demo_out/runs/sweeps/<cell>/ per cell, plus a
sweep_summary.tsv listing duration/exit code per cell.
"""
...
SWEEP_ROOT = os.path.join(RUNS_ROOT, "sweeps")
SUMMARY_TSV = os.path.join(SWEEP_ROOT, "sweep_summary.tsv")
...
def run_dir(name: str) -> str:
    return os.path.join(SWEEP_ROOT, name)


def trainer_run_name(name: str) -> str:
    return f"runs/sweeps/{name}"
...
def log_candidates(name: str) -> list[str]:
    ...
    return [
        ...
        # Backward compatibility for pre-cleanup sweep outputs.
...
def run_one(cfg, p_drop: float, suffix: str, device: str, allow_cpu: bool) -> tuple[int, float]:
...
def main():
    ...
    summary_path = (
        ...
        else os.path.join(SWEEP_ROOT, f"sweep_summary{suffix}.tsv")
    )
    ...
    with open(summary_path, "a") as sf:
        ...
        for i, cfg in enumerate(sweep):
            ...
            rc, dt = run_one(cfg, p_drop, suffix, args.device, args.allow_cpu)

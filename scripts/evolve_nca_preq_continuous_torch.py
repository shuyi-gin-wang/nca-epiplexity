"""
PyTorch port of scripts/evolve_nca_preq_continuous.py.
...
Run from repo root:
    .venv/Scripts/python.exe scripts/evolve_nca_preq_continuous_torch.py
"""
import argparse
import os
import sys
...
from utils.nca_torch import NCANetworkTorch, _wrap_pad
...
def main():
    ...
    parser.add_argument("--probe-init", type=str, default="identity",
                        ...
                        help="identity: probe init'd to copy-paste (preq_length∝learnable structure beyond identity). "
                             "random: probe init'd Kaiming-uniform (preq_length∝probe's whole learning curve, "
    ...
    base_out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "demo_out", "runs")
    if args.run_name:
        out_dir = os.path.join(base_out, args.run_name)
    else:
        out_dir = os.path.dirname(os.path.abspath(__file__)) + "/demo_out"  # legacy flat
